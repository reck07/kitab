            ...
            <div className="editor-inline-toolbar">
              ...
              <div className="toolbar-scroll-wrap" ref={toolbarRef}
                onPointerDown={(e) => {
                  ...
                  longPressTimer.current = setTimeout(() => {
                    ...
                    if (tool) { onTogglePinTool(tool); btn.dataset.lp = '1'; }
                  }, 400);
                }}
                onPointerUp={(e) => {
              ...
              >
                <div className="toolbar-pinned">
                  {pinnedTools.includes('bold') && (<button className="inline-tool-btn" data-tool="bold" onClick={() => { editorRef.current?.focus(); document.execCommand('bold'); }} title="Bold" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('bold'); }}><Bold size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('timestamp') && (<button className="inline-tool-btn" data-tool="timestamp" onClick={handleTimestamp} title="Timestamp" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('timestamp'); }}><Clock size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('italic') && (<button className="inline-tool-btn" data-tool="italic" onClick={() => { editorRef.current?.focus(); document.execCommand('italic'); }} title="Italic" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('italic'); }}><Italic size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('h1') && (<button className="inline-tool-btn" data-tool="h1" onClick={() => { editorRef.current?.focus(); document.execCommand('formatBlock', false, 'h1'); editorRef.current?.focus(); }} title="Heading 1" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('h1'); }}><Heading1 size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('list') && (<button className="inline-tool-btn" data-tool="list" onClick={() => { editorRef.current?.focus(); document.execCommand('insertUnorderedList'); }} title="Bullet List" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('list'); }}><List size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('undo') && (<button className="inline-tool-btn" data-tool="undo" onClick={handleUndo} title="Undo" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('undo'); }}><Undo2 size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('redo') && (<button className="inline-tool-btn" data-tool="redo" onClick={handleRedo} title="Redo" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('redo'); }}><Redo2 size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('h2') && (<button className="inline-tool-btn" data-tool="h2" onClick={() => { editorRef.current?.focus(); document.execCommand('formatBlock', false, 'h2'); editorRef.current?.focus(); }} title="Heading 2" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('h2'); }}><Heading2 size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('quote') && (<button className="inline-tool-btn" data-tool="quote" onClick={() => { editorRef.current?.focus(); document.execCommand('formatBlock', false, 'blockquote'); }} title="Quote" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('quote'); }}><MessageSquare size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('numlist') && (<button className="inline-tool-btn" data-tool="numlist" onClick={() => { editorRef.current?.focus(); document.execCommand('insertOrderedList'); }} title="Numbered List" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('numlist'); }}><CheckSquare size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('hr') && (<button className="inline-tool-btn" data-tool="hr" onClick={() => { editorRef.current?.focus(); document.execCommand('insertHorizontalRule'); }} title="Divider" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('hr'); }}><span className="toolbar-hr-icon">—</span></button>)}
                  {pinnedTools.includes('image') && (<button className="inline-tool-btn" data-tool="image" onClick={() => fileInputRef.current?.click()} title="Image" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('image'); }}><ImageIcon size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('voice') && (<button className={`inline-tool-btn${isRecording ? ' recording' : ''}`} data-tool="voice" onClick={handleVoiceInput} title="Voice Input" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('voice'); }}><Mic size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('emoji') && (<button className="inline-tool-btn" data-tool="emoji" onClick={() => setShowEmojiPicker(!showEmojiPicker)} title="Emoji" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('emoji'); }}><Smile size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('grammar') && (<button className="inline-tool-btn" data-tool="grammar" onClick={handleGrammarCheck} title="Grammar" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('grammar'); }}><SpellCheck size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('search') && (<button className={`inline-tool-btn${showSearch ? ' active' : ''}`} data-tool="search" onClick={() => { setShowSearch(!showSearch); if (!showSearch) setTimeout(() => searchInputRef.current?.focus(), 100); }} title="Search" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('search'); }}><Search size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('theme') && (<button className="inline-tool-btn" data-tool="theme" onClick={onThemeChange} title="Theme" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('theme'); }}><Sun size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('focus') && (<button className={`inline-tool-btn${focusMode ? ' active' : ''}`} data-tool="focus" onClick={onToggleFocus} title="Focus" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('focus'); }}><Layout size={16} strokeWidth={2} /></button>)}
                  {!activeNote.isRecycled && pinnedTools.includes('note-pin') && (<button className={`inline-tool-btn${activeNote.isPinned ? ' active' : ''}`} data-tool="note-pin" onClick={() => onTogglePin?.(activeNote.id)} title={activeNote.isPinned ? 'Unpin' : 'Pin'} onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('note-pin'); }}><Pin size={16} strokeWidth={2} fill={activeNote.isPinned ? 'currentColor' : 'none'} /></button>)}
                  {!activeNote.isRecycled && pinnedTools.includes('fave') && (<button className={`inline-tool-btn${activeNote.isFavorite ? ' active' : ''}`} data-tool="fave" onClick={() => onToggleFavorite?.(activeNote.id)} title={activeNote.isFavorite ? 'Unfavorite' : 'Favorite'} onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('fave'); }}><Star size={16} strokeWidth={2} fill={activeNote.isFavorite ? 'currentColor' : 'none'} /></button>)}
                  {!activeNote.isRecycled && pinnedTools.includes('archive') && (<button className="inline-tool-btn" data-tool="archive" onClick={() => onToggleArchive?.(activeNote.id)} title={activeNote.isArchived ? 'Unarchive' : 'Archive'} onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('archive'); }}><Archive size={16} strokeWidth={2} /></button>)}
                  {!activeNote.isRecycled && pinnedTools.includes('delete') && (<button className="inline-tool-btn" data-tool="delete" onClick={() => onDelete?.(activeNote.id)} title="Delete" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('delete'); }} style={{ color: '#e25c5c' }}><Trash2 size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('command') && (<button className="inline-tool-btn" data-tool="command" onClick={() => onAppAction?.('command')} title="Commands" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('command'); }}><Search size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('add') && (<button className="inline-tool-btn" data-tool="add" onClick={() => onAppAction?.('add')} title="New Note" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('add'); }}><FilePlus size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('exportPDF') && (<button className="inline-tool-btn" data-tool="exportPDF" onClick={() => onAppAction?.('exportPDF')} title="Print" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('exportPDF'); }}><Printer size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('exportJSON') && (<button className="inline-tool-btn" data-tool="exportJSON" onClick={() => onAppAction?.('exportJSON')} title="Export JSON" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('exportJSON'); }}><Download size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('share') && (<button className="inline-tool-btn" data-tool="share" onClick={handleShare} title="Share Note" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('share'); }}><Share2 size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('graph') && (<button className="inline-tool-btn" data-tool="graph" onClick={() => onAppAction?.('graph')} title="Knowledge Graph" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('graph'); }}><GitBranch size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('calculator') && (<button className="inline-tool-btn" data-tool="calculator" onClick={() => onAppAction?.('calculator')} title="Calculator" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('calculator'); }}><CalcIcon size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('drawing') && (<button className="inline-tool-btn" data-tool="drawing" onClick={() => onAppAction?.('drawing')} title="Drawing Pad" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('drawing'); }}><Pencil size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('lock') && (<button className="inline-tool-btn" data-tool="lock" onClick={() => onAppAction?.('lock')} title="Lock Note" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('lock'); }}><Lock size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('favorites') && (<button className="inline-tool-btn" data-tool="favorites" onClick={() => onAppAction?.('favorites')} title="Favorites" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('favorites'); }}><Star size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('trash') && (<button className="inline-tool-btn" data-tool="trash" onClick={() => onAppAction?.('trash')} title="Trash" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('trash'); }}><Trash2 size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('qrcode') && (<button className="inline-tool-btn" data-tool="qrcode" onClick={() => onAppAction?.('qrcode')} title="QR Code" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('qrcode'); }}><QrCode size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('layout') && (<button className="inline-tool-btn" data-tool="layout" onClick={() => onAppAction?.('layout')} title="Layout" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('layout'); }}><Layout size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('security') && (<button className="inline-tool-btn" data-tool="security" onClick={() => onAppAction?.('security')} title="Security" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('security'); }}><Shield size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('automation') && (<button className="inline-tool-btn" data-tool="automation" onClick={() => onAppAction?.('automation')} title="Automation" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('automation'); }}><Zap size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('refresh') && (<button className="inline-tool-btn" data-tool="refresh" onClick={handleMindRefresh} title="Mind Refresh" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('refresh'); }}><RotateCcw size={16} strokeWidth={2} /></button>)}
                  {pinnedTools.includes('settings') && (<button className="inline-tool-btn" data-tool="settings" onClick={() => onAppAction?.('settings')} title="Settings" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('settings'); }}><Cog size={16} strokeWidth={2} /></button>)}
                </div>
                ...
                <div className="toolbar-scroll">
                  ...
                  {!activeNote.isRecycled && (
                    <>
                      <span className="inline-tool-divider" />
                      {pinnedTools.includes('note-pin') && (<button className={`inline-tool-btn${activeNote.isPinned ? ' active' : ''}`} data-tool="note-pin" onClick={() => onTogglePin?.(activeNote.id)} title={activeNote.isPinned ? 'Unpin' : 'Pin'} onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('note-pin'); }}><Pin size={16} strokeWidth={2} fill={activeNote.isPinned ? 'currentColor' : 'none'} /></button>)}
                      {!pinnedTools.includes('note-pin') && (<button className={`inline-tool-btn${activeNote.isPinned ? ' active' : ''}`} data-tool="note-pin" onClick={() => onTogglePin?.(activeNote.id)} title={activeNote.isPinned ? 'Unpin' : 'Pin'} onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('note-pin'); }}><Pin size={16} strokeWidth={2} fill={activeNote.isPinned ? 'currentColor' : 'none'} /></button>)}
                      {pinnedTools.includes('fave') && (<button className={`inline-tool-btn${activeNote.isFavorite ? ' active' : ''}`} data-tool="fave" onClick={() => onToggleFavorite?.(activeNote.id)} title={activeNote.isFavorite ? 'Unfavorite' : 'Favorite'} onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('fave'); }}><Star size={16} strokeWidth={2} fill={activeNote.isFavorite ? 'currentColor' : 'none'} /></button>)}
                      {!pinnedTools.includes('fave') && (<button className={`inline-tool-btn${activeNote.isFavorite ? ' active' : ''}`} data-tool="fave" onClick={() => onToggleFavorite?.(activeNote.id)} title={activeNote.isFavorite ? 'Unfavorite' : 'Favorite'} onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('fave'); }}><Star size={16} strokeWidth={2} fill={activeNote.isFavorite ? 'currentColor' : 'none'} /></button>)}
                      {pinnedTools.includes('archive') && (<button className="inline-tool-btn" data-tool="archive" onClick={() => onToggleArchive?.(activeNote.id)} title={activeNote.isArchived ? 'Unarchive' : 'Archive'} onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('archive'); }}><Archive size={16} strokeWidth={2} /></button>)}
                      {!pinnedTools.includes('archive') && (<button className="inline-tool-btn" data-tool="archive" onClick={() => onToggleArchive?.(activeNote.id)} title={activeNote.isArchived ? 'Unarchive' : 'Archive'} onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('archive'); }}><Archive size={16} strokeWidth={2} /></button>)}
                      {pinnedTools.includes('delete') && (<button className="inline-tool-btn" data-tool="delete" onClick={() => onDelete?.(activeNote.id)} title="Delete" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('delete'); }} style={{ color: '#e25c5c' }}><Trash2 size={16} strokeWidth={2} /></button>)}
                      {!pinnedTools.includes('delete') && (<button className="inline-tool-btn" data-tool="delete" onClick={() => onDelete?.(activeNote.id)} title="Delete" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('delete'); }} style={{ color: '#e25c5c' }}><Trash2 size={16} strokeWidth={2} /></button>)}
                    </>
                  )}
                  <span className="inline-tool-divider" />
                  <div className="toolbar-overflow-wrap" ref={overflowRef}>
                    ...
                    {showOverflow && (
                      <div className="toolbar-overflow-dropdown">
                        <button className="overflow-tool-btn" data-tool="command" onClick={() => { onAppAction?.('command'); setShowOverflow(false); }} title="Commands" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('command'); }}><Search size={14} strokeWidth={2} /><span>Commands</span></button>
                        <button className="overflow-tool-btn" data-tool="add" onClick={() => { onAppAction?.('add'); setShowOverflow(false); }} title="New Note" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('add'); }}><FilePlus size={14} strokeWidth={2} /><span>New Note</span></button>
                        <div className="overflow-divider" />
                        <button className="overflow-tool-btn" data-tool="exportPDF" onClick={() => { onAppAction?.('exportPDF'); setShowOverflow(false); }} title="Print" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('exportPDF'); }}><Printer size={14} strokeWidth={2} /><span>Print</span></button>
                        <button className="overflow-tool-btn" data-tool="exportJSON" onClick={() => { onAppAction?.('exportJSON'); setShowOverflow(false); }} title="Export JSON" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('exportJSON'); }}><Download size={14} strokeWidth={2} /><span>Export JSON</span></button>
                        <button className="overflow-tool-btn" data-tool="share" onClick={() => { handleShare(); setShowOverflow(false); }} title="Share Note" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('share'); }}><Share2 size={14} strokeWidth={2} /><span>Share</span></button>
                        <button className="overflow-tool-btn" data-tool="graph" onClick={() => { onAppAction?.('graph'); setShowOverflow(false); }} title="Knowledge Graph" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('graph'); }}><GitBranch size={14} strokeWidth={2} /><span>Graph</span></button>
                        <button className="overflow-tool-btn" data-tool="calculator" onClick={() => { onAppAction?.('calculator'); setShowOverflow(false); }} title="Calculator" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('calculator'); }}><CalcIcon size={14} strokeWidth={2} /><span>Calculator</span></button>
                        <button className="overflow-tool-btn" data-tool="drawing" onClick={() => { onAppAction?.('drawing'); setShowOverflow(false); }} title="Drawing Pad" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('drawing'); }}><Pencil size={14} strokeWidth={2} /><span>Drawing</span></button>
                        <button className="overflow-tool-btn" data-tool="lock" onClick={() => { onAppAction?.('lock'); setShowOverflow(false); }} title="Lock Note" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('lock'); }}><Lock size={14} strokeWidth={2} /><span>Lock</span></button>
                        <div className="overflow-divider" />
                        <button className="overflow-tool-btn" data-tool="favorites" onClick={() => { onAppAction?.('favorites'); setShowOverflow(false); }} title="Favorites" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('favorites'); }}><Star size={14} strokeWidth={2} /><span>Favorites</span></button>
                        <button className="overflow-tool-btn" data-tool="trash" onClick={() => { onAppAction?.('trash'); setShowOverflow(false); }} title="Trash" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('trash'); }}><Trash2 size={14} strokeWidth={2} /><span>Trash</span></button>
                        <button className="overflow-tool-btn" data-tool="qrcode" onClick={() => { onAppAction?.('qrcode'); setShowOverflow(false); }} title="QR Code" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('qrcode'); }}><QrCode size={14} strokeWidth={2} /><span>QR Code</span></button>
                        <button className="overflow-tool-btn" data-tool="layout" onClick={() => { onAppAction?.('layout'); setShowOverflow(false); }} title="Layout" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('layout'); }}><Layout size={14} strokeWidth={2} /><span>Layout</span></button>
                        <button className="overflow-tool-btn" data-tool="security" onClick={() => { onAppAction?.('security'); setShowOverflow(false); }} title="Security" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('security'); }}><Shield size={14} strokeWidth={2} /><span>Security</span></button>
                        <button className="overflow-tool-btn" data-tool="automation" onClick={() => { onAppAction?.('automation'); setShowOverflow(false); }} title="Automation" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('automation'); }}><Zap size={14} strokeWidth={2} /><span>Automation</span></button>
                        <div className="overflow-divider" />
                        <button className="overflow-tool-btn" data-tool="refresh" onClick={() => { handleMindRefresh(); setShowOverflow(false); }} title="Mind Refresh" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('refresh'); }}><RotateCcw size={14} strokeWidth={2} /><span>Refresh</span></button>
                        <button className="overflow-tool-btn" data-tool="settings" onClick={() => { onAppAction?.('settings'); setShowOverflow(false); }} title="Settings" onContextMenu={(e) => { e.preventDefault(); onTogglePinTool('settings'); }}><Cog size={14} strokeWidth={2} /><span>Settings</span></button>
                      </div>
                    )}
                  </div>
                </div>
